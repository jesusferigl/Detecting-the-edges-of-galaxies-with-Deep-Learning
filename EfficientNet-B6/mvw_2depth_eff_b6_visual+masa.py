...
hyperparams = {
    ...
    'data_path_mass' : 'Imagenes_con_NaN/mass/',
    'epochs' : 2,
    'learning_rate' : 0.001,
...
class MultiviewNet(torch.nn.Module):
    ...
    def __init__(self, name_second_network):
        ...
        self.modelo = smp.Unet(encoder_name = name_second_network, encoder_weights = 'noisy-student', in_channels=45, classes=2)
    ...
    def forward(self, x1, x2, x3, x4, x5):
        ...
        output_after_red_1 = torch.cat([output_after_conv_2_1, output_after_conv_2_2, output_after_conv_2_3, output_after_conv_2_4, output_after_conv_2_5], axis = 1)
...
class MyDataset(Dataset):
    ...
    def __init__(self, path_visual_images, path_mass_images, path_masks):

        self.path_visual_images = path_visual_images
        self.path_mass_images = path_mass_images
    ...
    def __getitem__(self, index):
        ...
        x_masa = Image.open(self.path_mass_images + self.path_masks[index].split('/')[-1][:-6] + '.mass.png')
        ...
        x_masa = np.array(self.resize(x_masa))/65535
        ...
        x_masa = torch.from_numpy(x_masa).float().unsqueeze(dim = 0)

        return x_HJI, x_HJV, x_HIV, x_JIV, x_masa, y
...
class MyDatasetNormalRotationAndFlip(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if angulo == 90:
            ...
            x_masa = cv2.rotate(x_masa, cv2.ROTATE_90_CLOCKWISE)
            ...
            x_masa = cv2.rotate(x_masa, cv2.ROTATE_180)
            ...
            x_masa = cv2.rotate(x_masa, cv2.ROTATE_90_COUNTERCLOCKWISE)
        ...
        noise_x_masa = torch.from_numpy(abs(np.random.normal(loc=0, scale=np.random.choice([0.01, 0.05, 0.1]), size=x_masa.shape))).float()
        ...
        x_masa = (torch.from_numpy(x_masa).float() + noise_x_masa).unsqueeze(dim = 0)
...
train = MyDatasetNormalRotationAndFlip(hyperparams['data_path_HIJV'], hyperparams['data_path_mass'], train_masks)
valid = MyDataset(hyperparams['data_path_HIJV'], hyperparams['data_path_mass'], validation_masks)
...
def validation_metrics(neural_network):
    ...
    with torch.no_grad():
        ...
        for data in validloader:
            ...
            inputs1, inputs2, inputs3, inputs4, inputs5, labels = data[0].to(device), data[1].to(device), data[2].to(device), data[3].to(device), data[4].to(device), data[5].to(device)
            outputs = neural_network(inputs1, inputs2, inputs3, inputs4, inputs5)
...
def perform_train(neural_network):
    ...
    torch.save(best_model, 'best_model_MVW-2Depth-' + hyperparams['architecture'] + '-Mass')
    torch.save(neural_network, 'last_model_MVW-2Depth-' + hyperparams['architecture'] + '-Mass')
